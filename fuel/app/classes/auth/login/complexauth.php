...
class Auth_Login_ComplexAuth extends \Auth_Login_Driver {
	...
	public function perform_check()
	{
		...
		{
			$this->user = \DB::select()->where('username', '=', $username)->from(\Config::get('complexauth.table_name'))->execute();
	...
	public function login($username = '', $password = '')
	{
		...
		$this->user = \DB::select()
				...
				->from(\Config::get('complexauth.table_name'))->execute();
		if ($this->user->count() == 0)
		{
			return false;
		}
	...
	public function create_user($username, $password, $email, $group = 1, Array $profile_fields = array())
	{
		...
		$result = \DB::insert(\Config::get('complexauth.table_name'))
			->set($user)
			->execute();
	...
	public function update_user($values, $username = null)
	{
		...
		$affected_rows = \DB::update(\Config::get('complexauth.table_name'))
			->set($update)
			->where('username', '=', $username)
			->execute();
	...
	public function delete_user($username)
	{
		...
		$affected_rows = \DB::delete(\Config::get('complexauth.table_name'))
			->where('username', '=', $username)
			->execute();
	...
	public function create_login_hash()
	{
		...
		\DB::update(\Config::get('complexauth.table_name'))
	...
	public function get_user_groups()
	{
		...
		return array(array('complexgroup', $this->user->get('group')));
	}
