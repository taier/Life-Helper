...
function htmLawed($t, $config=1, $S=array())
{
...
// config eles
	...
	$x = !empty($config['elements']) ? str_replace(array("\n", "\r", "\t", ' '), '', $config['elements']) : '*';
	if ($x == '-*')
	{
		$e = array();
	}
	...
	$config['elements'] = & $e;
// config attrs
	$x = !empty($config['deny_attribute']) ? str_replace(array("\n", "\r", "\t", ' '), '', $config['deny_attribute']) : '';
	$x = array_flip((isset($x[0]) && $x[0] == '*') ? explode('-', $x) : explode(',', $x . (!empty($config['safe']) ? ',on*' : '')));
	if (isset($x['on*']))
	{
		unset($x['on*']);
	...
	$config['deny_attribute'] = $x;
// config URL
	$x = (isset($config['schemes'][2]) && strpos($config['schemes'], ':')) ? strtolower($config['schemes']) : 'href: aim, feed, file, ftp, gopher, http, https, irc, mailto, news, nntp, sftp, ssh, telnet; *:file, http, https';
	...
	foreach (explode(';', str_replace(array(' ', "\t", "\r", "\n"), '', $x)) as $v)
	{
		...
		if ($x2)
		{
			$config['schemes'][$x] = array_flip(explode(',', $x2));
		}
	}
...
// config rest
	...
	if ($config['clean_ms_char'])
	{
		...
		$x = $x + ($config['clean_ms_char'] == 1 ? array("\x82" => '&#8218;', "\x84" => '&#8222;', "\x91" => '&#8216;', "\x92" => '&#8217;', "\x93" => '&#8220;', "\x94" => '&#8221;') : array("\x82" => '\'', "\x84" => '"', "\x91" => '\'', "\x92" => '\'', "\x93" => '"', "\x94" => '"'));
		$t = strtr($t, $x);
	}
	if ($config['cdata'] or $config['comment'])
	{
	...
	if ($config['hook'])
	{
		$t = $config['hook']($t, $config, $S);
	}
	if ($config['show_setting'] && preg_match('`^[a-z][a-z0-9_]*$`i', $config['show_setting']))
	{
		$GLOBALS[$config['show_setting']] = array('config' => $config, 'spec' => $S, 'time' => microtime());
	}
// main
	$t = preg_replace_callback('`<(?:(?:\s|$)|(?:[^>]*(?:>|$)))|>`m', 'hl_tag', $t);
	$t = $config['balance'] ? hl_bal($t, $config['keep_bad'], $config['parent']) : $t;
	$t = (($config['cdata'] or $config['comment']) && strpos($t, "\x01") !== false) ? str_replace(array("\x01", "\x02", "\x03", "\x04", "\x05"), array('', '', '&', '<', '>'), $t) : $t;
	$t = $config['tidy'] ? hl_tidy($t, $config['tidy'], $config['parent']) : $t;
	unset($config, $e);
	if (isset($reC))
	{
		$GLOBALS['C'] = $reC;
...
// balance tags
// by content
	$configB = array('blockquote' => 1, 'form' => 1, 'map' => 1, 'noscript' => 1); // Block
	$configE = array('area' => 1, 'br' => 1, 'col' => 1, 'embed' => 1, 'hr' => 1, 'img' => 1, 'input' => 1, 'isindex' => 1, 'param' => 1); // Empty
	$configF = array('button' => 1, 'del' => 1, 'div' => 1, 'dd' => 1, 'fieldset' => 1, 'iframe' => 1, 'ins' => 1, 'li' => 1, 'noscript' => 1, 'object' => 1, 'td' => 1, 'th' => 1); // Flow; later context-wise dynamic move of ins & del to $configI
	$configI = array('a' => 1, 'abbr' => 1, 'acronym' => 1, 'address' => 1, 'b' => 1, 'bdo' => 1, 'big' => 1, 'caption' => 1, 'cite' => 1, 'code' => 1, 'dfn' => 1, 'dt' => 1, 'em' => 1, 'font' => 1, 'h1' => 1, 'h2' => 1, 'h3' => 1, 'h4' => 1, 'h5' => 1, 'h6' => 1, 'i' => 1, 'kbd' => 1, 'label' => 1, 'legend' => 1, 'p' => 1, 'pre' => 1, 'q' => 1, 'rb' => 1, 'rt' => 1, 's' => 1, 'samp' => 1, 'small' => 1, 'span' => 1, 'strike' => 1, 'strong' => 1, 'sub' => 1, 'sup' => 1, 'tt' => 1, 'u' => 1, 'var' => 1); // Inline
	$configN = array('a' => array('a' => 1), 'button' => array('a' => 1, 'button' => 1, 'fieldset' => 1, 'form' => 1, 'iframe' => 1, 'input' => 1, 'label' => 1, 'select' => 1, 'textarea' => 1), 'fieldset' => array('fieldset' => 1), 'form' => array('form' => 1), 'label' => array('label' => 1), 'noscript' => array('script' => 1), 'pre' => array('big' => 1, 'font' => 1, 'img' => 1, 'object' => 1, 'script' => 1, 'small' => 1, 'sub' => 1, 'sup' => 1), 'rb' => array('ruby' => 1), 'rt' => array('ruby' => 1)); // Illegal
	$configN2 = array_keys($configN);
	$configR = array('blockquote' => 1, 'dir' => 1, 'dl' => 1, 'form' => 1, 'map' => 1, 'menu' => 1, 'noscript' => 1, 'ol' => 1, 'optgroup' => 1, 'rbc' => 1, 'rtc' => 1, 'ruby' => 1, 'select' => 1, 'table' => 1, 'tbody' => 1, 'tfoot' => 1, 'thead' => 1, 'tr' => 1, 'ul' => 1);
	$configS = array('colgroup' => array('col' => 1), 'dir' => array('li'), 'dl' => array('dd' => 1, 'dt' => 1), 'menu' => array('li' => 1), 'ol' => array('li' => 1), 'optgroup' => array('option' => 1), 'option' => array('#pcdata' => 1), 'rbc' => array('rb' => 1), 'rp' => array('#pcdata' => 1), 'rtc' => array('rt' => 1), 'ruby' => array('rb' => 1, 'rbc' => 1, 'rp' => 1, 'rt' => 1, 'rtc' => 1), 'select' => array('optgroup' => 1, 'option' => 1), 'script' => array('#pcdata' => 1), 'table' => array('caption' => 1, 'col' => 1, 'colgroup' => 1, 'tfoot' => 1, 'tbody' => 1, 'tr' => 1, 'thead' => 1), 'tbody' => array('tr' => 1), 'tfoot' => array('tr' => 1), 'textarea' => array('#pcdata' => 1), 'thead' => array('tr' => 1), 'tr' => array('td' => 1, 'th' => 1), 'ul' => array('li' => 1)); // Specific - immediate parent-child
	$configO = array('address' => array('p' => 1), 'applet' => array('param' => 1), 'blockquote' => array('script' => 1), 'fieldset' => array('legend' => 1, '#pcdata' => 1), 'form' => array('script' => 1), 'map' => array('area' => 1), 'object' => array('param' => 1, 'embed' => 1)); // Other
	$configT = array('colgroup' => 1, 'dd' => 1, 'dt' => 1, 'li' => 1, 'option' => 1, 'p' => 1, 'td' => 1, 'tfoot' => 1, 'th' => 1, 'thead' => 1, 'tr' => 1); // Omitable closing
// block/inline type; ins & del both type; #pcdata: text
	...
	$eN = array('a' => 1, 'big' => 1, 'button' => 1, 'fieldset' => 1, 'font' => 1, 'form' => 1, 'iframe' => 1, 'img' => 1, 'input' => 1, 'label' => 1, 'object' => 1, 'ruby' => 1, 'script' => 1, 'select' => 1, 'small' => 1, 'sub' => 1, 'sup' => 1, 'textarea' => 1); // Exclude from specific ele; $configN values
...
// $in sets allowed child
	...
	elseif (isset($configI[$in]))
	{
		$inOk = $eI;
		$configI['del'] = 1;
		$configI['ins'] = 1;
	}
	elseif (isset($configF[$in]))
	{
		$inOk = $eF;
		unset($configI['del'], $configI['ins']);
	}
	elseif (isset($configB[$in]))
	{
		$inOk = $eB;
		unset($configI['del'], $configI['ins']);
	}
	if (isset($configO[$in]))
	{
		$inOk = $inOk + $configO[$in];
	}
	if (isset($configN[$in]))
	{
		$inOk = array_diff_assoc($inOk, $configN[$in]);
	}
	...
	for ($i = -1, $configi = count($t); ++$i < $configi;)
	{
		...
		if ($ql = count($q))
		{
			...
			elseif (isset($configI[$p]))
			{
				$ok = $eI;
				$configI['del'] = 1;
				$configI['ins'] = 1;
			}
			elseif (isset($configF[$p]))
			{
				$ok = $eF;
				unset($configI['del'], $configI['ins']);
			}
			elseif (isset($configB[$p]))
			{
				$ok = $eB;
				unset($configI['del'], $configI['ins']);
			}
			...
			if (isset($configN[$p]))
			{
				$ok = array_diff_assoc($ok, $configN[$p]);
			}
		}
		else
		{
		...
		if ($s)
		{
			if (isset($configE[$e]) or !in_array($e, $q))
			{
				continue;
			} // Empty/unopen
			if ($p == $e)
			...
			for ($j = -1, $configj = count($q); ++$j < $configj;)
			{
				if (($d = array_pop($q)) == $e)
				{
					break;
		...
		// open tag
		// $configB ele needs $eB ele as child
		if (isset($configB[$e]) && strlen(trim($x)))
		{
			$t[$i] = "{$e}{$a}>";
		...
		if ((($ql && isset($configB[$p])) or (isset($configB[$in]) && !$ql)) && !isset($eB[$e]) && !isset($ok[$e]))
		{
		...
		if (!$ql or !isset($eN[$e]) or !array_intersect($q, $configN2))
		{
			if (!isset($ok[$e]))
			{
				if ($ql && isset($configT[$p]))
				{
					echo '</', array_pop($q), '>';
					unset($e, $x);
					--$i;
		...
		for ($k = -1, $kc = count($q); ++$k < $kc;)
		{
			...
			if (isset($configN[$d]))
			{
				$ok2 = array_diff_assoc($ok2, $configN[$d]);
			}
			if (!isset($ok2[$e]))
			{
				if (!$k && !isset($inOk[$e]))
...
// end
	...
	if (isset($x[0]))
	{
		if (strlen(trim($x)) && (($ql && isset($configB[$p])) or (isset($configB[$in]) && !$ql)))
		{
			echo '<div>', $x, '</div>';
		}
...
// comment/CDATA sec handler
	$t = $t[0];
	global $config;
	if (!($v = $config[$n = $t[3] == '-' ? 'comment' : 'cdata']))
	{
		return $t;
	}
	if ($v == 1)
...
// entitity handler
	...
	if ($t[0] != '#')
	{
		return ($config['and_mark'] ? "\x06" : '&') . (isset($U[$t]) ? $t : (isset($N[$t]) ? (!$config['named_entity'] ? '#' . ($config['hexdec_entity'] > 1 ? 'x' . dechex($N[$t]) : $N[$t]) : $t) : 'amp;' . $t)) . ';';
	}
	...
	{
		return ($config['and_mark'] ? "\x06" : '&') . "amp;#{$t};";
	}
	return ($config['and_mark'] ? "\x06" : '&') . '#' . (((ctype_digit($t) && $config['hexdec_entity'] < 2) or !$config['hexdec_entity']) ? $n : 'x' . dechex($n)) . ';';
// eof
}

function hl_prot($p, $config=null)
{
// check URL scheme
	global $config;
	$b = $a = '';
	...
	$config = isset($config['schemes'][$config]) ? $config['schemes'][$config] : $config['schemes']['*'];
	static $d = 'denied:';
	if (isset($config['!']) && substr($p, 0, 7) != $d)
	{
		$p = "$d$p";
	}
	if (isset($config['*']) or !strcspn($p, '#?;') or (substr($p, 0, 7) == $d))
	{
		return "{$b}{$p}{$a}";
	} // All ok, frag, query, param
	if (preg_match('`^([a-z\d\-+.&#; ]+?)(:|&#(58|x3a);|%3a|\\\\0{0,4}3a).`i', $p, $m) && !isset($config[strtolower($m[1])]))
	{ // Denied prot
		return "{$b}{$d}{$p}{$a}";
	}
	if ($config['abs_url'])
	{
		if ($config['abs_url'] == -1 && strpos($p, $config['base_url']) === 0)
		{ // Make url rel
		...
		{ // Make URL abs
			if (substr($p, 0, 2) == '//')
			{
				$p = substr($config['base_url'], 0, strpos($config['base_url'], ':') + 1) . $p;
			}
			elseif ($p[0] == '/')
			{
				$p = preg_replace('`(^.+?://[^/]+)(.*)`', '$1', $config['base_url']) . $p;
			}
			elseif (strcspn($p, './'))
			{
				$p = $config['base_url'] . $p;
...
// close tag
	...
	if (!empty($m[1]))
	{
		return (!isset($eE[$e]) ? "</$e>" : (($config['keep_bad']) % 2 ? str_replace(array('<', '>'), array('&lt;', '&gt;'), $t) : ''));
	}
...
// depr attrs
	if ($depTr)
	{
		...
		foreach ($a as $k => $v)
		{
			...
			{
				unset($a['bordercolor']);
				$config[] = 'border-color: ' . $v;
			}
			elseif ($k == 'clear')
			{
				unset($a['clear']);
				$config[] = 'clear: ' . ($v != 'all' ? $v : 'both');
			}
			elseif ($k == 'compact')
			{
				unset($a['compact']);
			...
			{
				unset($a[$k]);
				$config[] = $k . ': ' . ($v[0] != '*' ? $v . (ctype_digit($v) ? 'px' : '') : 'auto');
			}
			elseif ($k == 'hspace')
			{
				unset($a['hspace']);
				$config[] = "margin-left: {$v}px; margin-right: {$v}px";
			}
			...
			elseif ($k == 'name')
			{
				if ($config['no_deprecated_attr'] == 2 or ($e != 'a' && $e != 'map'))
				{
					unset($a['name']);
				}
			...
			{
				unset($a['noshade']);
				$config[] = 'border-style: none; border: 0; background-color: gray; color: gray';
			}
			elseif ($k == 'nowrap')
			{
				unset($a['nowrap']);
			...
			elseif ($k == 'type')
			{
				...
				$config[] = 'list-style-type: ' . (isset($ol_type[$v]) ? $ol_type[$v] : 'decimal');
			}
			elseif ($k == 'vspace')
			{
				unset($a['vspace']);
				$config[] = "margin-top: {$v}px; margin-bottom: {$v}px";
			}
		}
		if (count($config))
		{
			$config = implode('; ', $config);
			$a['style'] = isset($a['style']) ? rtrim($a['style'], ' ;') . '; ' . $config . ';' : $config . ';';
		}
	}
// unique ID
	if ($config['unique_ids'] && isset($a['id']))
	{
		if (!preg_match('`^[A-Za-z][A-Za-z0-9_\-.:]*$`', ($id = $a['id'])) or (isset($GLOBALS['hl_Ids'][$id]) && $config['unique_ids'] == 1))
		{
			unset($a['id']);
		}
		else
...
// Tidy/compact HTM
	...
	$config = array('caption' => 1, 'dd' => 1, 'dt' => 1, 'h1' => 1, 'h2' => 1, 'h3' => 1, 'h4' => 1, 'h5' => 1, 'h6' => 1, 'isindex' => 1, 'label' => 1, 'legend' => 1, 'li' => 1, 'object' => 1, 'p' => 1, 'pre' => 1, 'td' => 1, 'textarea' => 1, 'th' => 1);
